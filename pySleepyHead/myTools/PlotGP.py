...
def gp_plot(mo, mv, filename, fig_files=None, plot_title=None, fig_list=None, ref_str='_ref', test_str='_test'):
    ...
    plt.legend(loc=1)
    plt.subplot(234)
    plq(plt, mv, 'time', mv, 'o_is_quiet_sure', add=4, color='blue', linestyle='-', label='o_is_quiet_sure' + test_str + '+4')
    plq(plt, mv, 'time', mv, 'g_is_quiet_sure', add=4, color='red', linestyle='--', label='g_is_quiet_sure' + test_str + '+4')
    plq(plt, mv, 'time', mv, 'o_is_quiet', add=2, color='blue', linestyle='-', label='o_is_quiet' + test_str + '+2')
    plq(plt, mv, 'time', mv, 'g_is_quiet', add=2, color='red', linestyle='--', label='g_is_quiet' + test_str + '+2')
    plq(plt, mv, 'time', mv, 'o_quiet', color='blue', linestyle='-', label='o_quiet' + test_str + '')
    plq(plt, mv, 'time', mv, 'g_quiet', color='red', linestyle='--', label='g_quiet' + test_str + '')
    plq(plt, mv, 'time', mv, 'g_quiet', add= 2, color='black', linestyle='-', label='g_quiet' + test_str + '+2')
    plq(plt, mv, 'time', mv, 'G_QUIET_THR', add= 2, color='black', linestyle=':', label='G_QUIET_THR' + test_str + '+2')
    plq(plt, mv, 'time', mv, 'G_QUIET_THR', slr=-1, add= 2, color='black', linestyle=':', label='G_QUIET_THR' + test_str + '+2')
    plq(plt, mv, 'time', mv, 'o_quiet', add= 2, color='green', linestyle='-', label='o_quiet' + test_str + '+2')
    plq(plt, mv, 'time', mv, 'O_QUIET_THR', add= 2, color='green', linestyle=':', label='O_QUIET_THR' + test_str + '+2')
    plq(plt, mv, 'time', mv, 'O_QUIET_THR', slr=-1, add= 2, color='green', linestyle=':', label='O_QUIET_THR' + test_str + '+2')
    plt.legend(loc=1)
    plt.subplot(235)
    plq(plt, mv, 'time', mv, 'x_raw', color='red', linestyle='--', label='x_raw' + test_str)
    plq(plt, mv, 'time', mv, 'y_raw', color='red', linestyle='-.', label='y_raw' + test_str)
