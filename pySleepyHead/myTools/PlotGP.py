...
def gp_plot(mo, mv, filename, fig_files=None, plot_title=None, fig_list=None, ref_str='_ref', test_str='_test'):
    ...
    plq(plt, mv, 'time', mv, 'conf', color='magenta', linestyle=':', label='conf' + test_str)
    # plq(plt, mo, 'time', mo, 'eye_voltage_thr', color='red', linestyle='--', label='eye_voltage_thr' + ref_str)
    plt.legend(loc=1)
    plt.subplot(222)
    plq(plt, mo, 'time', mo, 'eye_cl', add=2, color='blue', linestyle='-', label='eye_cl' + ref_str + '2')
    plq(plt, mv, 'time', mv, 'eye_cl', add=2, color='cyan', linestyle='--', label='eye_cl' + test_str + '2')
    plq(plt, mo, 'time', mo, 'conf', color='green', linestyle='-.', label='conf' + ref_str)
    plq(plt, mv, 'time', mv, 'conf', color='magenta', linestyle=':', label='conf' + test_str)
    plt.legend(loc=1)
    plt.subplot(223)
